...
- 12 基于风险评估来规划审计
- 107 风险评估，可千万不要顾名思义。它不只是评估风险这一个环节，包括确定风险评估的目标，范围，方法，收集组织信息系统的信息，分析信息，确定风险，评估风险，编写报告，监控风险，这些都是风险评估的环节

### 审计软件

- 28 通用审计软件，就是检查以前的数据和记录是否正确；嵌入式审计软件，意思就是内嵌到系统中，然后在系统运行的时候，就可以实时进行监控了
- 90 计算机辅助软件工具，不是，计算机辅助审计工具(CAAT)；嵌入式数据收集工具，是实时收集交易信息；启发式扫描工具，是扫描病毒的
- 144 嵌入审计模块，主要目的是监控收集交易处理过程中的数据和证据，然后把这些数据和证据存储到审计日志中，供审计人员分析

### 职责分离

- 33 管理层不了解具体的指责；组织架构图不一定更新；面谈和观察是比较符合美式思维的；测试用户访问权限，这个不一定所有的职责分离都是通过访问权限来实现的
- 126 也是和IT经理讨论
- 81 只记录日志，不对日志做评审，不是一种有效的控制

### 审计争议，工作思路

- 50 先重新验证支持本次审计结果的证据；如果证据不充分，再重新进行审计测试；不要一上来就想着报告高层，这样会让高层觉得你不专业，不负责任
- 80 审计标准要求审计师收集充分且适当的证据。所以这一步是先确定是偶发事件，还是周期性系统性故障
  
### 流程控制设计测试/符合性测试

- 101 浏览审查/穿行测试是验证控制设计上，是否设计的好，设计的完善，适合；面谈，问询，重新执行测试，是验证这个设计有没有被很好的落实，执行，这是符合性测试；注意区分
- 131 有效性，就是符合性测试，然后这题会计利息计算，数据上的东西，重新计算验证一下比较好；有日志记录的，就可以审查日志
  
### 抽样

- 105 统计抽样/非统计抽样；属性抽样/变量抽样是统计抽样；判断抽样是非统计抽样；统计抽样更严谨一些
